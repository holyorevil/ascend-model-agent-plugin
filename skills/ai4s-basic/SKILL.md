...
description: >
   通用昇腾 NPU 模型迁移 Skill。适用于将基于 PyTorch / TensorFlow / vLLM 等框架的
   CUDA 项目迁移到华为昇腾 NPU（Ascend910 系列）。涵盖环境检查、代码分析、自动迁移、
   手动适配、分布式改造、CUDA 算子处理、第三方库替换、验证及 Skill 生成全流程。
   当用户提到 昇腾迁移、NPU适配、昇腾适配、CUDA转NPU、模型移植到华为NPU 时触发。
metadata:
   short-description: 通用昇腾 NPU 迁移落地手册
   category: NPU-Migration
   tags: [ascend, npu, migration, cuda, pytorch, tensorflow, vllm]
---
